...
export const ResetPassword: React.FC = () => {
  ...
  const resetPassword = useMutation((parms: ResetPasswordType) => axios.post('/auth/reset_password', parms), {
    onSuccess: () => {
    ...
    onError: (err: any) => {
      if (err.errors) {
        ...
        if(responseError[0].message === 'A password must be at least 8 characters')
          showStatus(responseError[0].message, 'error')
        else
          showStatus('You must match the password! ', 'error')
      } else {
        const error = err.response?.data?.error ?? ''
        if(error === 'no-password-reset') {
          showStatus('The password has not been updated. Please try again!', 'error')
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const formData  = {token: params?.token, password: formValues.password, password_confirmation: formValues.password_confirmation }
